...
public:
	...
	bool GenerateRandomHeightMap(ID3D11Device* device);
	bool Update();
	float* GetWavelength();
	void CheckCollisions(Camera* cam, Player* plr);
	float* GetAmplitude();
	bool SimplexNoiseHeightMap(ID3D11Device* device);
	void CheckCollectableCollision(Collectable _coin);
...
private:
	...
	bool Smoothing(ID3D11Device* device);
	double SimplexNoise(double xIn, double yIn);
	void SimplexNoiseHeightMap();
	float fBm(double x, double y);
...
private:
	...
	int perm[512];
	RayCollisionDetection *m_collisionDetection;
